...
#define CONTROLFLAGS_IS_RUNNING(flags) (((flags).load(std::memory_order_acquire) & CONTROLFLAGS_RUNNING) != 0)
...
#define CONTROLFLAGS_SET_RUNNING(flags) (void)((flags).fetch_or(CONTROLFLAGS_RUNNING, std::memory_order_release))
...
#define CONTROLFLAGS_CLEAR_RUNNING(flags) (void)((flags).fetch_and(~CONTROLFLAGS_RUNNING, std::memory_order_acquire))
