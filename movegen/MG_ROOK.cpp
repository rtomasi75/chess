	...
	/*	for (BB_SQUAREINDEX squareIndexFrom = 0; squareIndexFrom < COUNT_SQUARES; squareIndexFrom++)
		{
			...
			SLIDELOOKUP_Initialize(&pMoveGen->SlideLookUp[player][squareIndexFrom], mask);
			pMoveGen->SlideLookUp[player][squareIndexFrom].MoveBase = nextMove;
			...
			for (BB_SQUARECOUNT squareBit = 0; squareBit < count; squareBit++)
			{
				...
				pMoveInfo[move].KillMap = BITBOARD_EMPTY;
				...
				pMoveInfo[move].KillPiece = PIECETYPE_NONE;
				pMoveInfo[move].KillPlayer = PLAYER_NONE;
				pMoveInfo[move].KillDest = SQUAREINDEX_NONE;
			...
			for (MG_SLIDELOOKUPINDEX index = 0; index < pSlideTable->LookUpFrom[squareIndexFrom].TableSize; index++)
			{
				...
				const BB_BITBOARD targets = ROOK_QuietMovesFromSquare(squareFrom, occupancy);
